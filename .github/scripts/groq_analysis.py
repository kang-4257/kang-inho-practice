...
def main():
    ...
    prompt = (
        ...
        "- 특수 기호(◆, ◇ 등) 사용 금지.\n"
        "- 모든 설명은 반드시 한국어로 작성. CVE ID, 패키지명 등 기술 용어만 영어 사용.\n"
        "- 취약점 영문 설명을 그대로 출력하지 말고 반드시 한국어로 요약해서 작성.\n"
        ...
        "   - **원인 및 영향도**: 해당 취약점이 왜 발생하고 어떤 피해를 줄 수 있는지 2~3문장으로 한국어 설명\n"
        "   - **조치 방법**: 구체적인 해결 방법 한국어 설명\n"
        "     - Dockerfile 또는 requirements.txt 수정 예시 (반드시 포함):\n"
        "       ```\n"
        "       # Before: 취약한 버전 설명 주석\n"
        "       (취약한 코드)\n"
        "       # After: 패치된 버전으로 업데이트\n"
        "       (수정된 코드)\n"
        "       ```\n\n"
        "**Medium/Low 취약점 섹션 (Critical/High 유무와 관계없이 반드시 포함):**\n"
        "[ Medium/Low 취약점 대응 Best Practice ]\n"
        "1. **항목 제목**: 구체적인 설명과 적용 방법\n"
        "2. **항목 제목**: 구체적인 설명과 적용 방법\n"
        "3. **항목 제목**: 구체적인 설명과 적용 방법\n\n"
        "**종합 의견 (반드시 포함):**\n"
        "종합 의견\n"
        "- 현재 이미지의 전반적인 보안 상태 평가\n"
        "- 발견된 취약점 외에 추가로 고려해야 할 보안 사항 2~3가지\n"
        "- 향후 보안 강화를 위한 권고사항\n\n"
        "### 스캔 데이터\n"
        f"이미지 태그: {image_tag}\n"
